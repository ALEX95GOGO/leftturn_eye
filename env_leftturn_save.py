...
class LeftTurn(object):
    def __init__(self, enabled_obs_number=3, vehicle_type = 'random',
                 ...
                 conservative_surrounding = False, frame=12, port=2000, town=5):
    ...
    def step(self, action):
        ...
        state_space = self.camera_output[:, :, ::-1]
        ...
        if self.town==5:
            collision = (dis_1<5 or dis_2<5 or dis_3<5) and x_ego>-187
        if self.town ==1:
            finish = (y_ego>330) and (x_ego>103)
        
        if self.town ==5:
            finish = (x_ego<-191) and (y_ego>-80)
        ...
        if index_obs_concerned is None:
            index_obs_concerned = 0
        return next_observation, human_control, reward, self.intervention, done, physical_variables, self.risk, self.travel_dis, self.rgb, v_ego, self.obs_list[index_obs_concerned].get_velocity().x, state_space

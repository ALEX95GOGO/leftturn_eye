...
class LeftTurn(gym.Env):
    def __init__(self, enabled_obs_number=3, vehicle_type = 'random',
        ...
        self.state_ = np.zeros((3,100,300),dtype=np.float32)
    ...
    def reset(self):
        ...
        bp_seman_camera.set_attribute('image_size_x', '180')
        ...
        self.seman_camera = self.world.spawn_actor(
            self.cameras[1][-1],
            carla.Transform(carla.Location(x=2, z=1),carla.Rotation(yaw=300)),
            attach_to=self.ego_vehicle)
        ...
        self.seman_camera_3 = self.world.spawn_actor(
            self.cameras[1][-1],
            carla.Transform(carla.Location(x=2, z=1),carla.Rotation(yaw=60)),
            attach_to=self.ego_vehicle)
    ...
    def get_observation(self):
        ...
        # state variable sets
        state_space = np.concatenate([self.camera_output[:,:,0], self.camera_output_2[:,:,0], self.camera_output_3[:,:,0]], axis=0)

...
screen_width, screen_height = 1280//4, 720//4
#WIDTH, HEIGHT = 80, 45
...
class LeftTurn(gym.Env):
    def __init__(self, enabled_obs_number=3, vehicle_type = 'random',
        ...
        self.surface = None
        #self.camera_output = np.zeros([720,720,3])
        #self.camera_output_2 = np.zeros([720,720,3])
        #self.camera_output_3 = np.zeros([720,720,3])
        self.camera_output = np.zeros([100,100,3])
        self.camera_output_2 = np.zeros([100,100,3])
        self.camera_output_3 = np.zeros([100,100,3])
        ...
        self.state_ = np.zeros((100,300,3),dtype=np.float32)
        ...
        if self.town == 50 or self.town == 501 or self.town == 502:
            self.world = self.client.load_world('Town05')
        if self.town == 7:
        ...
        self.observation_space = spaces.Box(low=0, high=1, shape=(3,HEIGHT, WIDTH*3), dtype=np.float16)
    ...
    def reset(self):
        ...
        if self.town == 5:
            spawn_point_ego.location.x = -176
            spawn_point_ego.location.y = -92
            spawn_point_ego.location.z  = 0.1
        if self.town == 50 or self.town == 501 or self.town == 502:
            spawn_point_ego.location.x = -176
            spawn_point_ego.location.y = -92
        ...
        #initial_velocity = carla.Vector3D(0, 5, 0)
        ...
        if self.town == 502:
            bp_pedestrian = self.world.get_blueprint_library().filter('walker.pedestrian.0001')[0]
            spawn_point_pedestrian = spawn_point_ego
            spawn_point_pedestrian.location.x = -196
            spawn_point_pedestrian.location.y = -80
            spawn_point_pedestrian.location.z = 0.1
            spawn_point_pedestrian.rotation.yaw = 180
            self.pedestrian = self.world.spawn_actor(bp_pedestrian, spawn_point_pedestrian)
            self.pedestrian.set_target_velocity(carla.Vector3D(0,1,0))
            self.agent.set_destination(carla.Location(-191, -50, spawn_point_ego.location.z))
            
            ## spawn the surrounding vehicles
            self.obs_list, self.bp_obs_list, self.spawn_point_obs_list = [], [] ,[]
            self.obs_velo_list = []
            lat_list = [-185, -185, -185]
            long_list = [-85, -65, -45]
            ...
            for index in range(self.enabled_obs):
                ...
                self.obs_list[index].set_target_velocity(carla.Vector3D(-5,0,0))
                self.obs_velo_list.append(np.random.randint(4,6))
        ...
        if self.town == 501:
            ...
            lat_list = [-190, -200, -210]
            long_list = [-88, -88, -88]
    
                    
            for index in range(3):
                bp, sp = self._produce_vehicle_blueprint(lat_list[index], long_list[index], 0)
                self.bp_obs_list.append(bp)
                self.spawn_point_obs_list.append(sp)
            for index in range(3):
                self.obs_list.append(self.world.spawn_actor(self.bp_obs_list[index],
            ...
            for index in range(3):
                self.obs_list.append(self.world.spawn_actor(self.bp_obs_list[index],
                                                        self.spawn_point_obs_list[index+3]))
                self.obs_list[index].set_target_velocity(carla.Vector3D(-5,0,0))
        ...
        if self.town == 5:
            ...
            for index in range(self.enabled_obs):
                ...
                # self.world.tick()
                self.obs_velo_list.append(np.random.randint(2,4))
                
        
        if self.town == 1:
            self.agent.set_destination(carla.Location(120, 330, spawn_point_ego.location.z))
    
            ## spawn the surrounding vehicles
            self.obs_list, self.bp_obs_list, self.spawn_point_obs_list = [], [] ,[]
            self.obs_velo_list = []
            #lat_list = [110, 125, 145]
            lat_list = [105, 125, 145]
        ...
        ## configurate and spawn the camera sensors
        self.camera_transforms = [
            (carla.Transform(carla.Location(x=2.8, z=1.7),carla.Rotation(yaw=180, pitch=-20)), self.Attachment.SpringArm),
            (carla.Transform(carla.Location(x=-5, z=5), carla.Rotation(pitch=10)), self.Attachment.SpringArm)]
        ...
        bp_seman_camera.set_attribute('image_size_x', '100')
        bp_seman_camera.set_attribute('image_size_y', '100')
        #bp_seman_camera.set_attribute('sensor_tick', '0.04')
        bp_seman_camera.set_attribute('fov', '60')
        ...
        self.viz_camera = self.world.spawn_actor(
            self.cameras[1][-1],
            carla.Transform(carla.Location(x=2.8, z=1.7),carla.Rotation(yaw=330)),
            attach_to=self.ego_vehicle)
        ...
        self.viz_camera = self.world.spawn_actor(
            self.cameras[0][-1],
            carla.Transform(carla.Location(x=2, z=1),carla.Rotation(yaw=330)),
            attach_to=self.ego_vehicle)

        self.seman_camera = self.world.spawn_actor(
            self.cameras[1][-1],
            carla.Transform(carla.Location(x=2, y=-1,z=1),carla.Rotation(yaw=300)),
            attach_to=self.ego_vehicle)
            
        self.seman_camera_2 = self.world.spawn_actor(
            self.cameras[1][-1],
            carla.Transform(carla.Location(x=2, z=1),carla.Rotation(yaw=0)),
            attach_to=self.ego_vehicle)
            
        self.seman_camera_3 = self.world.spawn_actor(
            self.cameras[1][-1],
            carla.Transform(carla.Location(x=2,y=1, z=1),carla.Rotation(yaw=60)),
            attach_to=self.ego_vehicle)
            
        # obtain the camera image
        weak_self = weakref.ref(self)
        self.seman_camera.listen(lambda image: LeftTurn._parse_seman_image(weak_self, image))
        self.seman_camera_2.listen(lambda image: LeftTurn._parse_seman_image_2(weak_self, image))
        self.seman_camera_3.listen(lambda image: LeftTurn._parse_seman_image_3(weak_self, image))
        
        self.viz_camera.listen(lambda image: LeftTurn._parse_image(weak_self, image))
        #self.viz_camera.listen(lambda image: LeftTurn._parse_seman_image(weak_self, image))
    ...
    def _parse_seman_image_2(weak_self, image):
        self = weak_self()
        if not self:
            return
        image.convert(self.cameras[1][1])
        ...
        array = array[:, :, :3]
        array = array[:, :, ::-1]
        self.camera_output_2 = array
    
    def _parse_seman_image_3(weak_self, image):
        self = weak_self()
        if not self:
            return
        image.convert(self.cameras[1][1])
    ...
    def step(self, action):
        ...
        '''
        if self.intervention:
            self.control.throttle = np.float(abs(human_control)) if human_control>=0 else 0
            self.control.brake = np.float(abs(human_control)) if human_control<0 else 0

        else:
            self.control.throttle = np.float(abs(action)) if action>=0 else 0
        ...
        #print(self.ego_location)
        # record the human intervention histroy
        self.intervene_history.append(human_control)
        
        dis_step = ((self.ego_vehicle.get_location().x - self.ego_location.x)**2+(self.ego_vehicle.get_location().y-self.ego_location.y)**2)**(1/2)
        self.ego_location = self.ego_vehicle.get_location()
        self.travel_dis = self.travel_dis + dis_step
        ...
        dis_1 = ((self.ego_vehicle.get_location().x - self.obs_list[0].get_location().x)**2+(self.ego_vehicle.get_location().y-self.obs_list[0].get_location().y)**2)**(1/2)

        dis_2 = ((self.ego_vehicle.get_location().x - self.obs_list[1].get_location().x)**2+(self.ego_vehicle.get_location().y-self.obs_list[1].get_location().y)**2)**(1/2)
        dis_3 = ((self.ego_vehicle.get_location().x - self.obs_list[2].get_location().x)**2+(self.ego_vehicle.get_location().y-self.obs_list[2].get_location().y)**2)**(1/2)
        #print(y_ego)
        #if self.town == 1:
        #    collision = (dis_1<5 or dis_2<5 or dis_3<5) and y_ego<327
        
        #if self.town==5:
        #    collision = (dis_1<5 or dis_2<5 or dis_3<5) and x_ego>-188
        ...
        if self.town ==5:
            finish = (x_ego<-191) and (y_ego>-70)
            
        if self.town ==50 or self.town==501 or self.town==502:
            finish = (x_ego<-191) and (y_ego>-70)
            
        done = collision or finish or self.count>400
        
        
        
                
        if self.town==502:
            if self._dis_p_to_p(self.ego_vehicle.get_location().x, self.ego_vehicle.get_location().y, self.pedestrian.get_location().x, self.pedestrian.get_location().y)<25:
                pedestrian_control = self.pedestrian.get_control()
                #import pdb; pdb.set_trace()
                pedestrian_control.speed = 1
                self.pedestrian.apply_control(pedestrian_control)
                if self._dis_p_to_p(self.ego_vehicle.get_location().x, self.ego_vehicle.get_location().y, self.pedestrian.get_location().x, self.pedestrian.get_location().y)<3:
                    collision = 1
                    done = 1
        ...
        reward = finish*100 - collision*100 -(self.count>400)*100 - (v_ego<0.2)
        #reward = finish*10 - collision*100 -(self.count>400)*100 - 0.5*(v_ego<0.2)
            ...
            for index in range(len(self.obs_list)):
                if x_ego < (self.obs_list[index].get_location().x - 1.5):
                    ...
                    if v_ego > 0.1: # calculate ttc to the surrounding vehicle 
                        ttc = np.clip(dis/v_ego, 0,5)
                        penalty_list.append(ttc)
        if self.town == 502: # calculate ttc to the surrounding vehicle 
            ttc = np.clip(self._dis_p_to_p(self.ego_vehicle.get_location().x, self.ego_vehicle.get_location().y, self.pedestrian.get_location().x, self.pedestrian.get_location().y)/(v_ego+0.0001), 0,5)
            penalty_list.append(ttc)
        ...
        risk = min(penalty_list) if len(penalty_list) != 0 else 0
        risk = risk if y_ego<323.5 else 0
        self.risk = risk
        # penalty is added to the reward term
        #reward = reward + risk if risk is not None else reward + 0.5
        reward = reward + risk*0.1 if risk != 0 else reward + 0.5
        ...
        #or self.parse_events()
        if done:
            self.terminate_position = y_ego
            self.post_process()
        next_observation = next_observation.transpose(2,0,1)
        return next_observation, reward, done, physical_variables
        #return next_observation, human_control, reward, self.intervention, done, physical_variables, self.risk, self.travel_dis, self.rgb
    ...
    def get_observation(self):
        ...
        # state variable sets
        state_space = np.concatenate([self.camera_output[:,:,0], self.camera_output_2[:,:,0], self.camera_output_3[:,:,0]], axis=1)
        #state_space = cv2.resize(state_space,(3*WIDTH, HEIGHT))
        state_space = np.float16(np.squeeze(state_space)/255)
        
        #state = np.repeat(np.expand_dims(state_space,2), 3, axis=2)
        #state_ = state.copy()
        state_1 = self.state_[:,:,2].copy()
        self.state_[:,:,0] = self.state_[:,:,1].copy()
        self.state_[:,:,1] = state_1
        self.state_[:,:,2] = state_space.copy()
        
        # other indicators facilitating producing reward function signal
        other_indicators = None
        
        return self.state_, other_indicators

...
def train_leftturn_task():
    ...
    if args.algorithm == 0:
        ...
        from algo.TD3_BC import DRL    
        chekpoint_dir = '/data/zhuzhuan/VR_driving_data/data/script/leftturn_eye/algo/models_leftturn/td3_lag/actor1722866019.pkl'
    ...
    DRL_human = DRL(a_dim, s_dim, device=args.device)
    DRL = DRL(a_dim, s_dim, device=args.device)
    
    exploration_rate = args.initial_exploration_rate 
    
    if args.resume and os.path.exists(chekpoint_dir):
        checkpoint = torch.load(chekpoint_dir)
        DRL.load_actor(chekpoint_dir)
        start_epoch = 0
        args.maximum_episode = 20
    else:
        start_epoch = 0
    ...
    start_epoch = 1
    #h_model.load_state_dict(checkpoint['state_dict'])
    DRL_human.actor.load_state_dict(checkpoint['state_dict'])
    ...
    for i in range(start_epoch, args.maximum_episode):
        ...
        while not done:
            ...
            if not args.resume:
                action_fdbk, _ = DRL_human.choose_action(state)
            ...
            if total_step > learn_threshold and not args.resume:

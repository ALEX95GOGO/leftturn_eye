...
class DRL:
    ...
    def __init__(self, action_dim, state_dim, device='cuda', LR_C = LR_C, LR_A = LR_A):
        ...
        self.itera = 0
        self.alpha = 2.5 # weighted BC loss term
    ...
    def learn(self, batch_size = BATCH_SIZE, epoch=0, train=True, eye=False):
        ...
        if self.itera % self.policy_freq == 0:
            ...
            Q = -self.critic([states, pred_actions])[0] 
            ...
            print(F.mse_loss(pred_actions, actions_exp))
            if eye == True:
                if self.itera<500:
                    loss_actor = lmbda*Q + 0.1* loss_distance + kl_divergence*0.05/16 + F.mse_loss(pred_actions, actions_exp) 
                else:
                    loss_actor = lmbda*Q + 0.1* loss_distance + F.mse_loss(pred_actions, actions_exp) 
            else:
                loss_actor = lmbda*Q + 0.1* loss_distance + F.mse_loss(pred_actions, actions_exp) 
    ...
    def choose_action(self,state):
        ...
        #self.human_att_i = prob_distribution

...
from algo.network_model_lag import Actor,Critic
...
POLICY_FREQ = 1
NOISE_CLIP = 0.5
LR_CC = 0.0005  # Learning rate for the cost critic
...
class DRL:
    ...
    def __init__(self, action_dim, state_dim, device='cuda', LR_C = LR_C, LR_A = LR_A):
        ...
        self.replay_buffer = PrioritizedReplayBuffer(MEMORY_CAPACITY,
                                                  {"obs": {"shape": (45,80,3)},
                                                   ...
                                                   "done": {},
                                                   "distance": {},
                                                   "cost": {}},  # Add cost to replay buffer,
                                                  next_of=("obs"))
        ...
        self.cost_critic = Critic(self.state_dim, self.action_dim).to(self.device)  # Cost critic network
        self.cost_critic_target = Critic(self.state_dim, self.action_dim).to(self.device)
        self.cost_critic_optimizer = torch.optim.Adam(self.cost_critic.parameters(), LR_CC)

        self.lambda_ = torch.tensor(0.0, requires_grad=True, device=self.device)  # Lagrange multiplier
        self.lambda_optimizer = torch.optim.Adam([self.lambda_], lr=LR_I)
    ...
    def learn(self, batch_size = BATCH_SIZE, epoch=0):
        ...
        idxs = data['indexes']
        states, actions, actions_exp, costs = data['obs'], data['act'], data['acte'], data['cost']
        rewards = data['rew']
        ...
        costs = torch.FloatTensor(costs).to(self.device)  # Convert costs to tensor
        ...
        with torch.no_grad():
            ...
            target_c1, target_c2 = self.cost_critic_target([next_states, next_actions])
            target_c = torch.min(target_c1, target_c2)
            cost_expected = costs + (1 - dones) * self.gamma * target_c
        y_predicted1, y_predicted2 = self.critic.forward([states, actions]) 
        c_predicted1, c_predicted2 = self.cost_critic.forward([states, actions])
        ...
        loss_cost_critic = F.mse_loss(c_predicted1, cost_expected) + F.mse_loss(c_predicted2, cost_expected)
        ...
        if self.itera % self.policy_freq == 0:
            ...
            soft_update(self.cost_critic_target, self.cost_critic, self.tau)
        ...
        lambda_loss = -self.lambda_ * (cost_mean - 1.0)  # 1.0 is the constraint threshold for collision costs
        lambda_loss.backward()
        self.lambda_optimizer.step()
        self.lambda_.data.clamp_(min=0)  # Ensure lambda is non-negative
    ...
    def choose_action(self,state):
        ...
        #action, dis = self.actor.forward(state).detach()
        action = action.detach().squeeze(0).cpu().numpy()
        action = np.clip(action,-1, 1)

        return action, dis.detach()
    

    def store_transition(self,  s, a, ae, i, r, s_, dis, c, d=0):
